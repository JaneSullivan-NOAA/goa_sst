...
void model_parameters::userfunction(void)
{
  ...
  {
    if(srv_est(i,j)>0) obs(biom(yrs_srv(i),j),i,j); //>-1
  }
  }
  //For now - hard wire LL survey estimates
  for(int i=styr; i<=endyr; ++i)
  {
    LL_est(i,1) = exp(log_q_LL)*(exp(biom(i,1))+exp(biom(i,2))+exp(biom(i,3)));
    LL_est(i,2) = exp(log_q_LL)*(exp(biom(i,4))+exp(biom(i,5))+exp(biom(i,6)));
    LL_est(i,3) = exp(log_q_LL)*(exp(biom(i,7))+exp(biom(i,8))+exp(biom(i,9)));
  }
  for(int i=1; i<=nobs_LL; ++i) {
    if(srv_est_LL(i,1)>0) obs_LL1(biom(yrs_srv_LL(i),1), biom(yrs_srv_LL(i),2), biom(yrs_srv_LL(i),3), log_q_LL, i);
  }
  for(int i=1; i<=nobs_LL; ++i) {
    if(srv_est_LL(i,2)>0) obs_LL2(biom(yrs_srv_LL(i),4), biom(yrs_srv_LL(i),5), biom(yrs_srv_LL(i),6), log_q_LL, i);
  }
  for(int i=1; i<=nobs_LL; ++i) {
    if(srv_est_LL(i,3)>0) obs_LL3(biom(yrs_srv_LL(i),7), biom(yrs_srv_LL(i),8), biom(yrs_srv_LL(i),9), log_q_LL, i);
  }
  if (sd_phase()) 
  {
    biomA = exp(biom);
...
void SEPFUN1  model_parameters::obs_LL1(const dvariable& biom1, const dvariable& biom2, const dvariable& biom3, const dvariable& tmp_log_q, int i)
{
  begin_df1b2_funnel();
  ofstream& evalout= *pad_evalout;
  //dvariable pred_LL = exp(tmp_log_q) * (exp(biom1) + exp(biom2) + exp(biom3));
  jnll+=LL_wt*0.5*(yconst_LL(i,1) + square(log(exp(tmp_log_q) * (exp(biom1) + exp(biom2) + exp(biom3)))-log(srv_est_LL(i,1))) / yvar_LL(i,1));
  end_df1b2_funnel();
}

void SEPFUN1  model_parameters::obs_LL2(const dvariable& biom1, const dvariable& biom2, const dvariable& biom3, const dvariable& tmp_log_q, int i)
{
  begin_df1b2_funnel();
  ofstream& evalout= *pad_evalout;
  //dvariable pred_LL = exp(tmp_log_q) * (exp(biom1) + exp(biom2) + exp(biom3));
  jnll+=LL_wt*0.5*(yconst_LL(i,2) + square(log(exp(tmp_log_q) * (exp(biom1) + exp(biom2) + exp(biom3)))-log(srv_est_LL(i,2))) / yvar_LL(i,2));
  end_df1b2_funnel();
}

void SEPFUN1  model_parameters::obs_LL3(const dvariable& biom1, const dvariable& biom2, const dvariable& biom3, const dvariable& tmp_log_q, int i)
{
  begin_df1b2_funnel();
  ofstream& evalout= *pad_evalout;
  // dvariable pred_LL = exp(tmp_log_q) * (exp(biom1) + exp(biom2) + exp(biom3));
  jnll+=LL_wt*0.5*(yconst_LL(i,3) + square(log(exp(tmp_log_q) * (exp(biom1) + exp(biom2) + exp(biom3)))-log(srv_est_LL(i,3))) / yvar_LL(i,3));
  end_df1b2_funnel();
}
...
void model_parameters::final_calcs()
{
  ...
  dvar_vector biom_TOT_LL = exp(log_q_LL)*rowsum(biomA);
  dvar_vector SD_numer_LL = rowsum(elem_prod(exp(2*log(exp(log_q_LL)*exp(biomsd))+square(biomsd.sd)),(exp(square(biomsd.sd))-1)));
  dvar_vector SD_denom_LL = square(rowsum(exp(log(exp(log_q_LL)*exp(biomsd))+0.5*square(biomsd.sd))));
...
void   df1b2_pre_parameters::obs_LL1(const funnel_init_df1b2variable& biom1, const funnel_init_df1b2variable& biom2, const funnel_init_df1b2variable& biom3, const funnel_init_df1b2variable& tmp_log_q, int i)
{
  begin_df1b2_funnel();
  ofstream& evalout= *pad_evalout;
  //df1b2variable pred_LL = exp(tmp_log_q) * (exp(biom1) + exp(biom2) + exp(biom3));
  jnll+=LL_wt*0.5*(yconst_LL(i,1) + square(log(exp(tmp_log_q) * (exp(biom1) + exp(biom2) + exp(biom3)))-log(srv_est_LL(i,1))) / yvar_LL(i,1));
  end_df1b2_funnel();
}

void   df1b2_pre_parameters::obs_LL2(const funnel_init_df1b2variable& biom1, const funnel_init_df1b2variable& biom2, const funnel_init_df1b2variable& biom3, const funnel_init_df1b2variable& tmp_log_q, int i)
{
  begin_df1b2_funnel();
  ofstream& evalout= *pad_evalout;
  //df1b2variable pred_LL = exp(tmp_log_q) * (exp(biom1) + exp(biom2) + exp(biom3));
...
void   df1b2_pre_parameters::obs_LL3(const funnel_init_df1b2variable& biom1, const funnel_init_df1b2variable& biom2, const funnel_init_df1b2variable& biom3, const funnel_init_df1b2variable& tmp_log_q, int i)
{
  begin_df1b2_funnel();
  ofstream& evalout= *pad_evalout;
  // df1b2variable pred_LL = exp(tmp_log_q) * (exp(biom1) + exp(biom2) + exp(biom3));
